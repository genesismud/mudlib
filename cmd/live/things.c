...
#define COUNT_PARALYZE "_count_paralyze_"
...
manip_relocate_to(object item_o, object to)
{
	...
	if (dest == to)
	{
	    destmsg = query_verb() + " " + LANG_ASHORT(item_o) +
	    (query_verb() == "give" ? " to" : (dest->query_prop(CONT_I_ATTACH) ? " on" : " in"));
	    all2actbb(" tries to " + destmsg, ({ dest }), ", but fails.");
	    targetbb(" tries to " + destmsg + " you, but fails.", ({ dest }) );
	}
	else
	{
	    allbb(" tries to " + query_verb() + " " + LANG_ASHORT(item_o) + ", but fails.");
	}
    }
...
manip_set_whom(string prep, object *carr)
{
	...
	if (gDest->query_reject_gift(this_player()))
	{
	    notify_fail(gDest->query_The_name(this_player()) + " rejects your gift.\n");
	    gDest->catch_tell(this_player()->query_The_name(gDest) +
	      " tries to give you something.\n");
	    return 0;
	}
...
/*
 * Function name: stop_counting
 * Description:   This function is called if the player tries to stop counting.
 * Arguments:     mixed arg - If string extra string when player made a command
 *                if object, the time ran out on the paralyze.
 * Returns:       If this function should not allow the player to stop search
 *                it shall return 1.
 */
varargs int
stop_counting(mixed arg)
{
	...
	if (call[1] == "count_up")
	{
	    /* To verify we have the right alarm, check the player argument. */
	    if (call[4][0] == this_player())
	    {
		remove_alarm(call[0]);
	    }
	}
 ...
 * Arguments    : object player - the player who is counting
 *                object heap - the heap being counted.
 *                float delay - the delay between counts.
 *                int amount - how much we count per loop.
 *                int counted - how much we counted so far.
 */
void
count_up(object player, object heap, float delay, int amount, int counted)
{
    counted += amount;
    if (counted < heap->num_heap())
    {
	player->catch_tell("... " + counted + "\n");
	set_alarm(delay, 0.0, &count_up(player, heap, delay, amount, counted));
	return;
    }

    set_this_player(player);
    write("The last count reached " + heap->num_heap() + " " +
      heap->plural_short() + ".\n");
    say(QCTNAME(player) + " finishes " + player->query_possessive() +
      " count.\n");
    present(COUNT_PARALYZE, player)->remove_object();
}
...
count(string str)
{
    ...
    /* Not counting a heap. Display the number of items. */
    if (!obj->query_prop(HEAP_I_IS))
    {
	if (sizeof(obs) == 1)
	{
	    write("You find a single " + obj->short(this_player()) + ".\n");
	    return 1;
	}

	write("You count " + LANG_WNUM(sizeof(obs)) + " " + str + ".\n");
	return 1;
    }

    intg = max(this_player()->query_stat(SS_INT), 10);
    delay = 60.0 / itof(min(intg, 60));
    amount = 5 * (intg / 10);

    /* count_arg contains player, heap, interval, coins per count and total so far */
    set_alarm(delay, 0.0, &count_up(this_player(), obj, delay, amount, 0));

    /* Number of count cycles. */
    cycles = 1 + ((obj->num_heap() - 1) / amount);
    /* Seconds it will take to calculate the cycles, rounded up. */
    delay = (itof(cycles) * delay) + 1.999;

    setuid();
    seteuid(getuid());
    paralyze = clone_object(PARALYZE_OBJECT);
    paralyze->set_name(COUNT_PARALYZE);
    paralyze->set_standard_paralyze("counting");
    paralyze->set_stop_fun("stop_counting");
    paralyze->set_stop_verb("stop");
    paralyze->set_combat_stop(1);
    paralyze->set_stop_message("You stop counting the " +
      obj->plural_short(this_player()) + ".\n");
    paralyze->set_remove_time(ftoi(delay));
    paralyze->set_fail_message("You are busy counting the " +
      obj->plural_short(this_player()) +
      " and should 'stop' before doing something else.\n");
    paralyze->move(this_player(), 1);

    write("You start counting the " + obj->plural_short(this_player()) + ".\n");
...
get(string str)
{
    ...
    */
    ...
    if (parse_command(str, all_inventory(environment(this_player())), "[the] %i", itema))
    {
	...
	"[the] %s 'in' / 'inside' / 'from' [the] %i", items, cont))
    {
...
hide(string str)
{
    ...
    {
	notify_fail("You are not skilled enough to hide anything here.\n");
	return 0;
    }
	...
	foreach(object obj: itema)
	{
	    if (obj->query_prop(OBJ_I_LIGHT) > 0)
	    {
		msg += capitalize(LANG_THESHORT(obj)) + " is emitting light.\n";
		itema -= ({ obj });
	    }
	}
	if (!sizeof(itema))
...
look(string str, int brief)
{
    ...
    *obl,
    ...
    if (parse_command(str, environment(this_player()),
		      "'at' / 'in' / 'inside' [the] %i 'on' / 'carried' [by] [the] %i",
		      obarr, obl))
    {
    ...
    */
    ...
    if (!parse_command(str, ENV, "%w [the] %i", prp, obarr) ||
...
/* **************************************************************************
 * go - go somehwere, a long way of just typing the direction.
 */
int
go(string str)
{
    notify_fail("Go where?\n");
    if (!strlen(str))
	return 0;

    if (IN_ARRAY(str, environment(this_player())->query_exit_cmds()) ||
      IN_ARRAY(str, DEFAULT_DIRECTIONS))
    {
...
search(string str)
{
	...
	break;
    default:
	notify_fail("Be more specific. You can search only one thing at a time.\n");
	return 0;
    }
