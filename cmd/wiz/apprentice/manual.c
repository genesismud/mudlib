...
man(string entry)
{
    ...
    if (!entry)
    {
        notify_fail("For instructions on usage, do 'help man'.\n");
        return 0;
    }
    ...
    case "-k":
        if (argc == 2)
        {
            ...
            if (member_array(argv[1], chaparr) < 0)
            {
                ...
                {
                    man_arr = MANCTRL->get_keywords(chaparr[i], argv[1]);
                    if (sizeof(man_arr))
                    {
                        write("--- " + chaparr[i] + ":\n" +
                            sprintf("%-*#s\n", 76, implode(man_arr, "\n")) + "\n");
            ...
            {
                man_chapt = argv[1];
                man_arr = MANCTRL->get_index(man_chapt);
                g_mannum = 2;
                str = process_string("# 1:" + implode(man_arr, "@@man_num@@"));
        ...
        if (argc == 3)
        {
            ...
            man_chapt = argv[1];
            man_arr = MANCTRL->get_keywords(man_chapt, argv[2]);
            g_mannum = 2;
            str = process_string("# 1:" + implode(man_arr, "@@man_num@@"));
            write("Available subjects:\n" + sprintf("%-*#s\n", 76, str));
            break;
    ...
    default:
        ...
        if (argc == 2)
        {
            ...
            man_chapt = argv[0];
            man_arr = (mixed *)MANCTRL->get_keywords(man_chapt, argv[1]);
        }
        else
        {
            for (i = 0 ; i < sizeof(chaparr) ; i++)
            {
                man_chapt = chaparr[i];
                man_arr = (mixed *)MANCTRL->get_keywords(man_chapt, argv[0]);
                if (sizeof(man_arr) > 0)
                    break;
            }
            if (!sizeof(man_arr) && member_array(argv[0], chaparr) >= 0)
            {
                man_arr = MANCTRL->get_index(argv[0]);
                g_mannum = 2;
                str = process_string("# 1:" + implode(man_arr, "@@man_num@@"));
                write("Available subjects:\n" + sprintf("%-*#s\n", 76, str));
                return 1;
