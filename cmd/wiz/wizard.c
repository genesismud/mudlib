...
#include "/cmd/wiz/wizard/communication.c"
#include "/cmd/wiz/wizard/edit.c"
#include "/cmd/wiz/wizard/files.c"
...
cmdsoul(string str)
{
    ...
    /* Avoid mixup between cmdsouls and toolsouls. */
    if (str->query_tool_soul())
    {
        notify_fail("The command 'cmdsoul' cannot be used to add a tool " +
            "soul.\nUse 'toolsoul' for that.\n");
        return 0;
    }
...
magic_map(string str)
{
	...
	{
	    notify_fail("Syntax: map " + str + " <file>\n");
	    return 0;
	}
    }
    /* Check if the file exists, though you may remove a file that doesn't
    ...
    case "add":
    case "update":
	if (MAP_CENTRAL->add_map(mapfile))
	{
	    write("Successfully added " + mapfile + "\n");
	}
	return 1;

    case "coords":
	maps = explode(read_file(mapfile), "::NEWMAP::\n");
	write("Found " + sizeof(maps) + " map(s).\n");
	foreach(string newmap: maps)
	{
	    parts = explode(newmap, "\n::MAPDATA::\n");
	    if (sizeof(parts) != 3)
	    {
		write("Map \"" + newmap[..29] + "\" with " + sizeof(parts) + " part(s) skipped.\n");
		continue;
	    }
	    write("Coordinates for map: " + parts[0] + "\n");
	    lines = explode(lower_case(parts[2]), "\n");
	    size = sizeof(lines);
	    for (iy = 0; iy < size; iy++)
	    {
		result = "";
		chars = explode(lines[iy], "");
		xprev = 0;
		while((ix = member_array("o", chars)) >= 0)
		{
		    /* Only display the coordinates if they aren't part of a
		     * word. This is a bit of a kludge, but it simply checks
		     * the symbol before and the symbol after both aren't
		     * letters of the alphabet. */
		    if (((ix == 0) || (chars[ix-1] < "a") || (chars[ix-1] > "z")) &&
		        ((ix == (sizeof(chars) - 1)) || (chars[ix+1] < "a") || (chars[ix+1] > "z")))
		    {
		        result += ((ix+xprev) + "," + iy + " ");
		    }
		    xprev += (ix + 1);
    ...
    case "link":
	write("Linking map " + mapfile + "\n");
	MAP_CENTRAL->link_map(MASTER_OB(environment(this_interactive())), mapfile);
	return 1;

    case "list":
        if (mapfile == "here")
	{
	    str = implode(explode(MASTER_OB(environment(this_interactive())), "/")[..-2], "/") + "/";
	    lines = get_dir(str + "*.c");
	    lines = map(sort_array(lines), &extract(, 0, -3));
	    write("Directory " + str + " with " + sizeof(lines) + " file(s)\n");
	    foreach(string file: lines)
	    {
		mapfile = MAP_CENTRAL->query_maplink(str + file);
		write(sprintf("- %-20s %s\n", file, mapfile ? mapfile : "- no map -"));
	    }
	    return 1;
	}

        lines = MAP_CENTRAL->query_maplinks(mapfile);
	if (!sizeof(lines))
	{
	    write("No rooms link to: " + mapfile + "\n");
	    return 1;
	}
	write("Map " + mapfile + " is linked by " + sizeof(lines) + " file(s)\n  " +
	    implode(sort_array(lines), "\n  ") + "\n");
        return 1;

    case "remove":
	write("Removing all references to " + mapfile + "\n");
	MAP_CENTRAL->remove_map(mapfile);
	return 1;
	...
	foreach(string newmap: maps)
	{
	    ...
	    write("Showing test locations for map: " + parts[0] + "\n");
	    lines = explode(lower_case(parts[2]), "\n");
	    coords = explode(lower_case(parts[1]), "\n");
	    foreach(string coord: coords)
	    {
		if ((sscanf(coord, "%s %d %d %s", filename, ix, iy, str) != 4) &&
		    (sscanf(coord, "%s %d %d", filename, ix, iy) != 3))
		{
		    write("Cannot parse coordinates: " + coord + "\n");
		    continue;
		}
		if (iy >= sizeof(lines))
		{
		    write("Coordinates: " + coord + ": Y " + iy + " out of range.\n");
		    continue;
		}
		ix--;
		if (ix >= strlen(lines[iy]))
		{
		    write("Coordinates: " + coord + ": X " + (ix+1) + " out of range.\n");
		    continue;
		}
...
modify(string str)
{
    ...
    stat = member_array(words[1], SD_STAT_DESC);
    if ((stat < 0) ||
        (stat >= SS_NO_STATS))
    {
        notify_fail("Unknown stat name. Valid are: " +
            COMPOSITE_WORDS(SD_STAT_DESC) + ". Or use \"exp\" to change " +
...
msecond(string str)
{
	...
	if (sizeof(args) != 2)
	{
        ...
        if (!SECURITY->set_second_first(args[1]))
        {
            return 0;
        }
...
restrict(string str)
{
    ...
    rank = SECURITY->query_wiz_rank(who);

    if (rank < WIZ_STEWARD && SECURITY->query_mentor(args[0]) != who)
    {
        notify_fail("You are not the mentor of " +
	    capitalize(args[0]) + ".\n");
        return 0;
    }

    // For lieges, restrict the use of restrict (pun!) to domain members.
    if (!sizeof(doms = SECURITY->query_lord_domains(who)))
	doms = ({ SECURITY->query_wiz_dom(who) });

    if (args[0] != "list" && rank < WIZ_ARCH &&
	!IN_ARRAY(SECURITY->query_wiz_dom(args[0]), doms))
    {
	...
	{
            wlist = filter(wlist, &operator(!=)(-1) @
		&member_array(,doms) @ SECURITY->query_wiz_dom);
	}

        wlist = filter(wlist,
	    &operator(<=)(WIZ_NORMAL) @ SECURITY->query_wiz_rank);
	...
	foreach(string wiz: wlist)
	{
	    write(sprintf("%-11s%-11s",
		    capitalize(wiz), SECURITY->query_wiz_dom(wiz)));

	    res = SECURITY->query_restrict(wiz);
    ...
    {
        write(sprintf("%-11s%-11s",
		capitalize(args[0]), SECURITY->query_wiz_dom(args[0])));
    ...
    else
    {
        ...
        {
            notify_fail("It's not suitable to restrict a wizard of "+
...
shutdown_game(string str)
{
    ...
    if (argv[0] == "schedule")
    {
        ...
        if (strlen(str) < 17)
        {
            notify_fail("Timestamp must be of the format: mmm dd yyyy hh:mm[:ss]\n");
            return 0;
        }
        if (!(grace = STAMP2TIME(str)))
        {
            notify_fail("Not a recognised timestamp: " + str + ".\n");
            return 0;
        }

        if (grace < (time() + 86400))
        {
            notify_fail("The scheduled time must be at least 1 day in the future.\n");
            return 0;
        }
        SECURITY->set_uptime_limit(grace);
        write("Shutdown scheduled for " + ctime(grace) + " game time.\n");
        return 1;
    }
...
skillstat(string str)
{
    ...
    {
        write(capitalize(ob->query_real_name()) + " has no skills.\n");
        return 1;
    }
...
toolsoul(string str)
{
    ...
    /* Avoid mixup between cmdsouls and toolsouls. */
    if (str->query_cmd_soul())
    {
        notify_fail("The command 'toolsoul' cannot be used to add a regular " +
            "command soul.\nUse 'cmdsoul' for that.\n");
        return 0;
    }
