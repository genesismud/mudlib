...
pad(string str)
{
    ...
    case "trim":
        if (size != 1)
        {
            notify_fail("Syntax: pad trim\n");
            return 0;
        }

        read_pad_file();
        if (m_sizeof(pad_notes) < 2)
        {
            write("Your notepad does not need trimming.\n");
            return 1;
        }
	...
	mapping new_pad = ([ ]);
        while(++index < size)
	{
	    new_pad[index + 1] = pad_notes[indices[index]];
	}

	pad_notes = new_pad;
	new_pad = 0;

        write("Trimmed all gaps from your notepad.\n");
	save_pad_file();
        return 1;
	...
	// Updated to handle a SW of 'off', which == -1
        width = this_interactive()->query_option(OPT_SCREEN_WIDTH);
        width = (width > 0 ? width : 75);
        width = max(15, width - 15);
