...
#define WIZARD_S_NAME	"_wizard_s_name"
...
not_junior()
{
    string wname, name = this_player()->query_real_name();

    if (sscanf(this_player()->query_real_name(), "%sjr", wname) == 1)
    {
	string *arr = ({ wname }) + SECURITY->query_player_seconds();

	foreach (string who : arr)
	{
...
check_so_junior()
{
    ...
    {
	/* We have a valid wizard character, continue on. */
	if (SECURITY->query_wiz_rank(who) <= WIZ_RETIRED)
	    continue;

	this_player()->add_prop(WIZARD_S_NAME, who);
	return 1;
    }
...
clone_ob(string str)
{
     ...
     */
    name = this_player()->query_prop(WIZARD_S_NAME);
...
goto(string dest)
{
     ...
     * the name of the wizard.
     */
    name = this_interactive()->query_prop(WIZARD_S_NAME);
...
home(string str)
{
    ...
    {
	if (!stringp(str))
	    str = this_player()->query_prop(WIZARD_S_NAME);
...
update_ob(string str)
{
	...
	if (find_object(WIZ_CMD_NORMAL)->update_startloc_check(ob))
	{
...
slow_init_arg(string arg)
{
    ...
    if (catch(junior_shadow = clone_object(JUNIOR_SHADOW)))
    {
	environment()->catch_tell("Could not load the Junior Shadow!\n");
	return;
    }

    environment()->catch_tell("Trying to add the protecting shadow to " +
	"you ...\n");
